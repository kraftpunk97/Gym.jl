...
=#
get_torque(env::AcrobotEnv, action) = env.AVAIL_TORQUE[action]

function step!(env::AcrobotEnv, action)
    @assert action ∈ env.action_space "$(action) is not in the action space. Enter a valid action."
    s = env.state
    ...
    ns = vcat(wrap(ns[1:2], -π, π), bound(ns[3:4], -env.MAX_VEL_1, env.MAX_VEL_2)) # omit action
    env.state = ns
