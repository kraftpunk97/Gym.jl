...
function generate_random_map(shape=8, p=8f-1)
    ...
    while !valid
        p = min(1, p)
        res = sample(['F', 'H'], ProbabilityWeights(Float32[p, 1-p]), (shape, shape), replace=true)
        res[1, 1] = 'S'
        res[end, end] = 'G'
        valid = is_valid(res)
    end
...
function FrozenLakeEnv(desc_::AbstractArray{<:AbstractString, 1}, is_slippery::Bool)
    ...
    FrozenLakeEnv(desc, nrow, ncol, reward_range, discenv_obj)
end

function FrozenLakeEnv(map_name::Symbol, is_slippery::Bool=true)
    desc = MAPS[map_name]
    FrozenLakeEnv(desc, is_slippery)
end

function FrozenLakeEnv(is_slippery::Bool=true)
